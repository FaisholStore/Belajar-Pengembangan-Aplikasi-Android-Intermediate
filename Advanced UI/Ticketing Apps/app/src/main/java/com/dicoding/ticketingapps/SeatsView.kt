...
import com.dicoding.ticketingapps.Kelas.Seat
...
class SeatsView : View {
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        // Memanggil metode onMeasure dari kelas induk (superclass).
        super.onMeasure(widthMeasureSpec, heightMeasureSpec)

        // Mendapatkan lebar dan tinggi yang direkomendasikan.
        val width = getDefaultSize(suggestedMinimumWidth, widthMeasureSpec)
        val height = getDefaultSize(suggestedMinimumHeight, heightMeasureSpec)

        // Menghitung setengah tinggi dan setengah lebar objek tampilan.
        val halfOfHeight = height / 2
        val halfOfWidth = width / 2

        // Inisialisasi variabel 'value' dengan nilai -600F.
        var value = -600F

        // Melakukan perulangan dari 0 hingga 7.
        for (i in 0..7) {
            // Jika nilai i adalah genap (modulus 2 == 0), maka:
            if (i.mod(2) == 0) {
                // Mengatur posisi elemen ke-i (Kursi?) ke setengah lebar dikurangi 300F dan setengah tinggi ditambahkan 'value'.
                seats[i].apply {
                    x = halfOfWidth - 300F
                    y = halfOfHeight + value
                }
            } else {
                // Jika nilai i adalah ganjil, maka:
                // Mengatur posisi elemen ke-i ke setengah lebar ditambah 100F dan setengah tinggi ditambahkan 'value'.
                seats[i].apply {
                    x = halfOfWidth + 100F
                    y = halfOfHeight + value
                }
                // Menambahkan 'value' sebesar 300F (mungkin untuk menggeser elemen ke bawah).
                value += 300F
            }
        }
    }
